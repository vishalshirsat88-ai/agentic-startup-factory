# 🔥 DEBUG: SERVICE FILE GENERATED v4

def get_test_execution():
    test_pipeline_id = 123
    test_status = 'success'
    return {"test_status": test_status, "test_result": "PASSED"}

def get_test_execution_from_db():
    try:
        from engine.db import get_connection

        conn = get_connection()
        cursor = conn.cursor()

        cursor.execute(
            "SELECT name FROM items WHERE module = ?",  # Removed () around the parameter
        )

        rows = cursor.fetchall()
        conn.close()

        return [row[0] for row in rows]

    except Exception as e:
        return []

def execute():
    try:
        # 🔥 FIRST TRY DB
        data = get_test_execution_from_db()

        if data:
            result = {"data": data, "source": "database"}

        elif "get_test_execution" in globals():  # Added elif instead of else
            result = get_test_execution()
        else:
            result = {"message": "no data available"}

        return {
            "status": "success",
            "data": result,
            "error": None
        }

    except Exception as e:
        return {
            "status": "error",
            "data": None,
            "error": str(e)
        }

def add_test_execution(name):
    try:
        from engine.db import get_connection

        conn = get_connection()
        cursor = conn.cursor()

        cursor.execute(
            "INSERT INTO items (name, module) VALUES (?, ?)",
            (name, "test_execution")
        )
        conn.commit()
        conn.close()

        return {
            "status": "success",
            "data": {"message": "test_execution added"},
            "error": None
        }

    except Exception as e:
        return {
            "status": "error",
            "data": None,
            "error": str(e)
        }