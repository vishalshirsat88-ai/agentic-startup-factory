...
def landing():
    return render_template(
        ...
        feature_1_title="AI-powered focus training API (/focus-training), Mental wellness assessment and tracking API (/mental-wellness), Personalized gaming recommendations API (/game-recommendations), Social features to connect with fellow gamers API (/social), Rewards and achievements system API (/rewards), Onboarding flow for new users API (/onboarding), Daily focus training routine flow API (/focus-routine), Mental wellness assessment and tracking flow API (/wellness-tracking), Discovering new games and content flow API (/game-discovery), Reward redemption and achievement unlock flow API (/rewards-redemption)",
