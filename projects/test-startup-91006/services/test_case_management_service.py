...
def get_test_case_management():
    # Retrieve test case management details from database
    test_cases = [
        {"id": 1, "name": "Login functionality"},
        {"id": 2, "name": "Profile update functionality"}
    ]

    # Filter test cases with status 'active'
    active_test_cases = [case for case in test_cases if case['status'] == 'active']

    # Return test case management data
    return {
        "test_cases": active_test_cases,
        "total_count": len(active_test_cases),
        "status": "success"
    }
